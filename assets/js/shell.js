...
(function () {
  ...
  var PAGE = (function () {
    ...
    var file = (segs.pop() || 'index.html').replace('.html', '');
    var folder = segs[segs.length - 1];
    var subfolders = ['admin', 'seller', 'cs'];
    // Sertakan folder supaya 'admin/dashboard' != 'seller/dashboard' != 'cs/dashboard'
    return (subfolders.indexOf(folder) > -1 ? folder + '/' : '') + (file || 'index');
  })();
  var SW   = 214; /* sidebar width px */
  var TH   = 52;  /* topbar height px */

  /* ── BASE: path relatif ke folder public/ dari lokasi halaman saat ini ──
     Dipakai supaya link sidebar/topbar tetap benar baik saat shell.js
     dimuat dari public/*.html (depth 0) maupun public/admin|seller|cs/*.html (depth 1). */
  var BASE = (function () {
    var segs = location.pathname.split('/').filter(Boolean);
    segs.pop(); // buang nama file, sisanya folder
    var lastFolder = segs[segs.length - 1];
    var subfolders = ['admin', 'seller', 'cs'];
    return subfolders.indexOf(lastFolder) > -1 ? '../' : '';
  })();

  /* ── Halaman yang punya mobileHdr SENDIRI (tidak perlu sh-mob) ── */
  var HAS_OWN_HDR = [
    'deposit','riwayat','profil','seller/dashboard',
    'admin/dashboard','cs/dashboard','admin/info','admin/deposits'
  ];
  ...
  if (needMob) {
    ...
    mob.id = 'sh-mob';
    mob.innerHTML =
      '<a class="smb-brand" href="' + BASE + 'index.html">' +
  ...
  bar.id = 'sh-bar';
  bar.innerHTML =
    '<a class="sh-brand" href="' + BASE + 'index.html"><div class="sh-dot"></div>XREZZKY<em>STORE</em></a>' +
    '<div id="sh-srch">' +
    ...
    '<div id="sh-pw">' +
      ...
      '<div id="sh-dd">' +
        ...
        '<div class="d-sec">' +
          ...
          '<a class="d-a" href="' + BASE + 'profil.html"><i class="fas fa-user-circle"></i> Profil Saya</a>' +
          '<a class="d-a" href="' + BASE + 'profil.html?tab=pengaturan"><i class="fas fa-cog"></i> Pengaturan Akun</a>' +
        '</div>' +
  ...
  if (PAGE === 'index') {
    var srchBox = document.getElementById('sh-srch');
    var srchSp  = document.getElementById('sh-sp');
    if (srchBox) srchBox.style.display = 'flex';
    if (srchSp)  srchSp.style.display  = 'none';
    /* Handle ?q= dari URL */
    var urlQ = new URLSearchParams(location.search).get('q');
    if (urlQ) {
      var srchInp = document.getElementById('shSrchInp');
      if (srchInp) srchInp.value = urlQ;
  ...
  function buildSidebar(role) {
    ...
    h += sa('fas fa-home',    'Beranda',      BASE + 'index.html',   PAGE==='index');
    h += sa('fas fa-bell',    'Info & Update',BASE + 'index.html#info', false);
    h += sa('fas fa-wallet',  'Deposit',      BASE + 'deposit.html', PAGE==='deposit');
    h += sa('fas fa-receipt', 'Riwayat',      BASE + 'riwayat.html', PAGE==='riwayat');

    if (isSeller) {
      h += '<div class="s-sep"></div><div class="s-lbl">Seller</div>';
      h += sa('fas fa-store',         'Seller Dashboard', BASE + 'seller/dashboard.html',            PAGE==='seller/dashboard');
      h += sa('fas fa-boxes-stacked', 'Produk Saya',      BASE + 'seller/dashboard.html?tab=products', false);
      h += sa('fas fa-inbox',         'Pesanan Masuk',    BASE + 'seller/dashboard.html?tab=orders', false, 'sbOrd');
    }
    if (isAdmin) {
      h += '<div class="s-sep"></div><div class="s-lbl">' + (isOwner?'Owner':'Admin') + '</div>';
      h += sa('fas fa-chart-pie', 'Admin Panel',    BASE + 'admin/dashboard.html', PAGE==='admin/dashboard');
      h += sa('fas fa-newspaper', 'Info Admin',     BASE + 'admin/info.html',      PAGE==='admin/info');
      h += sa('fas fa-qrcode',    'Kelola Deposit', BASE + 'admin/deposits.html',  PAGE==='admin/deposits', 'sbDep', 'r');
    }
    if (isCs) {
      h += '<div class="s-sep"></div><div class="s-lbl">Customer Service</div>';
      h += sa('fas fa-headset', 'CS Dashboard', BASE + 'cs/dashboard.html', PAGE==='cs/dashboard');
    }

    side.innerHTML = h;

    /* Info & Update — kalau di index, buka info sheet */
    var infoLink = side.querySelector('[href="' + BASE + 'index.html#info"]');
    if (infoLink) {
